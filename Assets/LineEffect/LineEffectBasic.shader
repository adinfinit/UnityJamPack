...
Properties {
	...
	_Low("Low", Range(0, 15)) = 1
	_High("High", Range(0, 15)) = 8
}
SubShader {
	Pass {
		CGPROGRAM
		...
		float4 frag(v2f_img i) : COLOR {
			...
			float alpha = x.a + 
				...
				a4.a + b4.a + c4.a + d4.a;
			if(x.a <= 0.3f){
				result.rgb = _BackgroundColor;
			} else if(alpha < _Low){
				result.rgb = lerp(_BackgroundColor, _LineColor, alpha / _Low);
			} else if (_High < alpha) {
				result.rgb = lerp(_LineColor, _BackgroundColor, clamp(alpha - _High, 0.0f, 1.0f));
			} else {
